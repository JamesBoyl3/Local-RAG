...
class RAGModel:
    ...
    def create(
        cls,
        gen_model_loc: str | Path,
        embedding_model_loc: str | Path,
        doc_db_loc: str | Path = Path("./documents.db"),
        vector_db_loc: str | Path = Path("./embedding.faiss"),
    ) -> Self:
        llm = LLM(Path(gen_model_loc))
        doc_db = SQLManager(Path(doc_db_loc))
        vector_db = FAISSManager(index_path=Path(vector_db_loc))
        ingestion_pipeline = IngestionPipeline(
            EmbeddingModel(Path(embedding_model_loc))
        )
    ...
    def generate_response(self, query: str) -> str:
        ...
        self._conversation.add_message(
            "assistant", answer := self._llm.answer(messages)
        )
